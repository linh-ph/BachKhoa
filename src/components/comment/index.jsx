...
import "./style.scss";
import { editCommentPost, deleteCommentPost } from "../../slices/user";
import { getPostComments } from "../../slices/blog";
...
const Comment = (props) => {
  ...
  const handleMenuClick = (item) => {
    if (item.key === "1") {
      ...
      confirm({
        ...
        cancelText: "No",
        onOk() {
          dispatch(deleteCommentPost(comment.id))
            .unwrap()
            .then(() => {
  ...
  const onFinish = (values) => {
    ...
    dispatch(editCommentPost({ commentId: comment.id, data: data }))
      .unwrap()
      .then(() => {
